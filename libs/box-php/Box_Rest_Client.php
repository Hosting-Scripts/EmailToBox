...
class Box_Rest_Client {
	...
	/**
	 * Allows to set the auth_token to be used when buildin an upload url ... or
	 * in any other call.
	 * @param string $authToken A user auth_token valid for your api key
	 */
	public function set_auth_token($authToken) {
	...
	/**
	 * The authenticate() method attempts to manage the whole flow. This method
	 * simply gets a ticket for your api key and returns it leaving the follow-on
	 * redirects up to your code to handle.
	 * @return string A ticket to be used to obtain a user auth_token
	 */
	...
	public function delete($target, $target_id) {
		if (!in_array($target, array('file','folder'))) {
		...
		$res = $this->post('delete',$params);	
		return $res['status'];
	}


	public function add_comment($target, $target_id, $message) {
		if (!in_array($target, array('file','comment'))) {
			throw new Exception('Invalid $target');
		}
		$params = array(
			'target' => $target,
		...
		$res = $this->post('add_comment',$params);	
		return $res['status'];
	}
	 ...
	 */
	public function upload_url($type = 'upload', $id = 0, $uploadCreatesNewFile = false) {
		$url = '';
		switch(strtolower($type)) {
			case 'upload':
				$url = $this->upload_url.'/'.$this->api_version.'/upload/'.$this->auth_token.'/'.$id;
				if ($uploadCreatesNewFile) $url .= "?new_copy=1";
				break;
			case 'overwrite':
	...
	public function upload(Box_Client_File &$file, array $params = array()) {
		...
		else if(intval($file->attr('id')) !== 0 && !$params['new_copy']) {
			// This file is overwriting another
			$url = $this->upload_url('overwrite',$file->attr('id'));
		}
		else {
			// This file is a new upload
			$url = $this->upload_url('upload', $file->attr('folder_id'), (array_key_exists('new_copy', $params) && $params['new_copy']));
		}
